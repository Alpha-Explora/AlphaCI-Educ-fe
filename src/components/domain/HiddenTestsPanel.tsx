...
import type { HiddenTestFile, HiddenTestMode } from "@/models/types";
import {
  Banner,
  Button,
  Card,
...
export function HiddenTestsPanel({ assignmentId }: { assignmentId: string }) {
  ...
  return (
    <Card className="p-5">
      <SectionHeading
        title="Hidden tests"
        subtitle="Your own tests, which students cannot read. They catch solutions written to satisfy the visible tests rather than the problem."
        action={
          suite ? (
            ...
            <GenericPill tone="neutral">None uploaded</GenericPill>
          )
        }
      />
        ...
        <>
          {suite && (
            <p className="mt-3 text-xs text-[var(--text-muted)]">
              Last updated {formatDateTime(suite.uploadedAt)} ·{" "}
              {suite.mode === "secure" ? "Exam mode" : "Fast feedback"} ·{" "}
              {suite.revealed
                ? "published to students"
                : suite.revealAfterDue
                  ? "publishes after the due date"
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            {suite && (
              <Button variant="ghost" onClick={vm.remove} disabled={vm.isRemoving}>
                {vm.isRemoving ? "Removing…" : "Remove"}
              </Button>
            )}

            {suite && !vm.files && (
              <Button variant="ghost" onClick={vm.loadContent} disabled={vm.isLoadingContent}>
